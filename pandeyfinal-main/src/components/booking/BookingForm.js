...
export function BookingForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const bookingData = {
      ...
      bookingPackage: selectedPackage.title, // Renamed to bookingPackage
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/bookings`, {
        ...
        body: JSON.stringify(bookingData), // Send form data in the request body
      });

      if (response.ok) {
        console.log('Booking successfully submitted');
        setIsSubmitted(true); // Set submitted flag to true
        // Redirect to Razorpay link after form submission
        window.location.href = 'https://pages.razorpay.com/pl_PgV7mF8ATwq6FC/view'; // Redirect to Razorpay URL
      } else {
        console.error('Failed to submit booking');
      }
    } catch (error) {
      console.error('Error submitting booking:', error); // Log the error
    }
  };
  ...
  const isFormValid = () => {
    if (step === 1) {
      return formData.name && formData.phoneNumber; // Validate PersonalDetails
    }
    if (step === 2) {
      return formData.accommodationType; // Validate AccommodationDetails
    }
    if (step === 3) {
      return formData.checkIn; // Validate DateSelection (check-in only)
    }
    if (step === 4) {
      return formData.adults; // Validate GuestDetails (adults)
    }
    return true;
  };
